...
targetdata$dependent = gsub('bl.total.dtiMD.FiberAtlas.fibers','MD (reversed)',targetdata$dependent)
...
fig.depre=
  ...
  ggtitle("Depressive symptoms (by parents)")+
  theme(
...
fig.depre=
  ...
  geom_errorbar(aes(x=reorder(dependent,-ord), 
                    ymin=beta-std, ymax=beta+std), width=0.2, colour="grey", alpha=0.9, size=0.4)+
  ggtitle("Depressive symptoms (by children)")+
  theme(
    panel.background = element_blank(),
    panel.grid = element_blank(),
    axis.title.y = element_blank(),
...
tiff("Figs/SuppleInfo/harshQC_pANDy_report_depre_IM_bulk.tiff", width = 7, height = 6, units = 'in', res = 300)
Fig.total # Make plot
dev.off()
