...
# Settings
load('result/x.Supplementary_materials/DiffsYouthDepree_to_brain.RData')
...
fig.depre=
  ...
  geom_bar(fill='slateblue2',  position=position_dodge(), stat="identity", width=0.8) +
  geom_errorbar(aes(x=reorder(dependent,-ord), 
                    ymin=beta-std, ymax=beta+std), width=0.2, colour="grey", alpha=0.9, size=0.4)+
  ggtitle("Depressive symptoms: absolute difference\nbetween caregiver and child report")+
  theme(
...
tiff("Figs/SuppleInfo/DiffsYouthDepression_IM_bulk.tiff", width = 4, height = 3, units = 'in', res = 300)
fig.depre # Make plot
dev.off()
