...
res.1 = res.1[grepl('FiberAtlas',res.1$dependent),]
res.2 = filter(res.2,factor!='CBCL.dsm5.depre.p')
res.2 = res.2[grepl('FiberAtlas',res.2$dependent),]
...
fig.beta=ggplot(fig.dat.beta, aes(x=res2.beta, 
  ...
  ylab('Beta/Cohen\'s d: With extra QC')+
...
fig.p=ggplot(fig.dat.p, aes(x=-log10(res2.p.value), 
  ...
  ylab('-log10(P-value): With extra QC')+
  geom_smooth(method=lm,size=0.3)
...
png("Figs/SuppleInfo/ExtraQC_MainModel_correlation.png", width = 12, height = 5, units = 'in', res = 300)
sumPlot # Make plot
dev.off()
