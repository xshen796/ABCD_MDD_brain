...
res.1=res.1 %>% wholeB_correction(TargetResult = .,ls.factor = unique(.$factor),ls.dep = ls.dep)
res.2=res.2 %>% wholeB_correction(TargetResult = .,ls.factor = unique(.$factor),ls.dep = ls.dep)
rm(TargetResult)

# Select CBCL results. Remove vol.ASEG (subcortical volumes)

res.1 = res.1[!grepl('vol.ASEG',res.1$dependent),]
res.2 = filter(res.2,factor=='KSADS.Depressive_symptoms_ever.p')
...
fig.beta=ggplot(fig.dat.beta, aes(x=res2.beta, 
  ...
  xlab('Beta: Not controlling for\ncomorbidity (main model)')+
  ylab('Beta: Controlling for\ncomorbidity')+
...
png("Figs/SuppleInfo/CovComorbid_MainModel_correlation.png", width = 12, height = 5, units = 'in', res = 300)
sumPlot # Make plot
dev.off()
