...
# Mother
targetdata = filter(result.wholeBcorrected,factor=='depression_bioMother_noManiaVision')
...
# Father
targetdata = filter(result.wholeBcorrected,factor=='depression_bioFather_noManiaVision')
targetdata = targetdata[!grepl('vol.ASEG',targetdata$dependent),]
...
# Parents
targetdata = filter(result.wholeBcorrected,factor=='depression_bioParents_noManiaVision')
targetdata = targetdata[!grepl('vol.ASEG',targetdata$dependent),]
...
tiff("Figs/SuppleInfo/famRisk_pplot.tiff", width = 9, height = 16, units = 'in', res = 300)
fig.total # Make plot
dev.off()
