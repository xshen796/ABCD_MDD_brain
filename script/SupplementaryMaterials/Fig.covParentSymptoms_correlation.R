...
# Settings
load('result/x.Supplementary_materials/YouthDepree_WM_subcor_FS_covParentDepre.RData')
...
fig.beta=ggplot(fig.dat.beta, aes(x=res2.beta, 
  ...
  ylab('Beta/Cohen\'s d: ASR controlled')+
...
fig.p=ggplot(fig.dat.p, aes(x=-log10(res2.p.value), 
  ...
  ylab('-log10(P-value): ASR controlled')+
  geom_smooth(method=lm,size=0.3)
...
png("Figs/SuppleInfo/covParentSymptoms_MainModel_correlation.png", width = 12, height = 5, units = 'in', res = 300)
sumPlot # Make plot
dev.off()
