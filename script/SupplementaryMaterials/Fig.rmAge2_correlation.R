...
# Settings
load('result/x.Supplementary_materials/YouthDepree_WM_subcor_FS_noAge2.RData')
...
fig.beta=ggplot(fig.dat.beta, aes(x=res2.beta, 
  ...
  xlab('Beta/Cohen\'s d: Main result')+
  ylab('Beta/Cohen\'s d: Age-squared removed from model')+
...
fig.p=ggplot(fig.dat.p, aes(x=-log10(res2.p.value), 
  ...
  xlab('-log10(P-value): Main results')+
  ylab('-log10(P-value): Age-squared removed from model')+
  geom_smooth(method=lm,size=0.3)
...
png("Figs/SuppleInfo/rmAge2_MainModel_correlation.png", width = 12, height = 5, units = 'in', res = 300)
sumPlot # Make plot
dev.off()
