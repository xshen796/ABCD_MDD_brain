...
if(!sumsum){
      for(basePhen in 1:length(base.phenotypes.names)){
            ...
            if(dosage){
                  ...
                  ##if( ((covary = T & !is.na(user.covariate.file))) | (covary = F  )) 
            ...
            #  if(!dosage){
            #    if(os == "mac"){
            #      download.file(url ="https://www.cog-genomics.org/static/bin/plink140807/plink_mac.zip",
            #        destfile = paste(plinkpath, "PLINK_", Sys.Date(), ".zip", sep = "") ,
            #        method = "curl",
            #        quiet = T,
            #        extra="-k")
            #    }
            #    if(os == "linux"){
            #      download.file(url = "https://www.cog-genomics.org/static/bin/plink140802/plink_linux_i686.zip",
            #        destfile = paste(plinkpath, "PLINK_", Sys.Date(), ".zip", sep = "") ,
            #        method = "curl",
            #        quiet = T)
            #    }
            ...
            #  if(dosage){
            #    if(os == "mac"){
            #      download.file(url ="http://pngu.mgh.harvard.edu/~purcell/plink/dist/plink-1.07-mac-intel.zip",
            #        destfile = paste(plinkpath, "PLINK_", Sys.Date(), ".zip", sep = "") ,
            #        method = "curl",
            #        quiet = T,
            #        extra="-k")
            #    }
            #    if(os == "linux"){
            #      download.file(url = "http://pngu.mgh.harvard.edu/~purcell/plink/dist/plink-1.07-x86_64.zip",
            #        destfile = paste(plinkpath, "PLINK_", Sys.Date(), ".zip", sep = "") ,
            #        method = "curl",
            #        quiet = T)
            #    }
            #  }
            #  system(paste("unzip ", plinkpath, "PLINK_", Sys.Date(), ".zip ",  sep  =""))
...
#             }
#             
#             system(cat('sed -i "/_Physical.Position_/d" TARGET_SNPs'))
            ...
            ### --- implement dosages in plink2!
            ## read in dosage data in plink2
            #	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3   --fam ", target.fam))
            ## recode dosage to oxford for recoding -- doesn't work
            #	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3   --fam ", target.fam, "--recode oxford "))
            ## convert oxford to hard calls, for clumping? doesn't work..	
            #	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3 --hard-call-threshold 0.9 --make-bed   --fam ", target.fam))
            ## clump dosages - this doesn't work, need hard calls
            #	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3   --fam ", target.fam, " --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb) )
            ## PRS in plink2  - this works
            #	system(paste(plink," --allow-no-sex --noweb --dosage ", gen.name ,"  noheader	skip0=1	skip1=1	skip2=0 	format=3   --fam ", target.fam, "  --score rawfile.raw   --q-score-range rangelist.txt rangelist_ranges") )
            ...
            if(!multiple.target.phenotypes){
                  ...
                  if(!binary.target){
                        for(i in 1:length(lists$V1)){
                              ...
                              #	      nsnps[i] <-	      nsnps[i] <- dim(ComAlLi[ComAlLi$P < lists$V1[i],])[1]
                              nsnps[i] <- dim(ranges.list[ranges.list$V2 < lists$V1[i],])[1]
                              if(i %in% (seq(1, length(lists$V1), 1)[seq(1, length(lists$V1), 1) %in% ceiling(length(lists$V1)*seq(0, 1,0.1))] )){
                                    cat(paste("Regression Models: " ,round(100*(i/length(lists$V1)),digits=-1), "% Complete \n",sep=""))
                              }
            ...
            if(emp.pval){
                  ...
                  #          p.out <- apply(data.frame(p.out), 1, function(x){sum(p.perm < x) / length(p.perm)})
