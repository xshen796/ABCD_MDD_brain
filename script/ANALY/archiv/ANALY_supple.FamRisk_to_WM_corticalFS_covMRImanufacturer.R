...
ls.models$covs=paste0(c('interview_age','I(interview_age^2)','sex','fsqc_qu_motion','site_id_l','race_ethnicity','recent_socialdprv','scanner_manufacturer_pd'),collapse='+')#'involuntary_unemployment','household_income','ADHD_ever','recent_socialdprv',
...
result.famRisk.bulk=reg_phewasStyle(ls.model.bulk,dat_short=targetdata,dat_long=dat_long,correctByFactor = T)
result.famRisk.region=reg_phewasStyle(ls.model.region,dat_short=targetdata,dat_long=dat_long,correctByFactor = T)
result.famRisk.region.covWholeB=reg_phewasStyle(ls.model.region.covWholeB,dat_short=targetdata,dat_long=dat_long,correctByFactor = T)

save(result.famRisk.bulk,result.famRisk.region,result.famRisk.region.covWholeB,file='result/x.Supplementary_materials/famRisk_covMRIManufacturer.RData')
