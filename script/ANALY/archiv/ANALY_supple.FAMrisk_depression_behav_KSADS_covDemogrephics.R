...
# dependent variables
ls.dep.behav=colnames(targetdata)[grep('^KSADS\\.',colnames(targetdata))]
...
ls.models$covs=paste0(c('interview_age','I(interview_age^2)','gender','race_ethnicity','site_id_l','recent_socialdprv','household_income'),
...
diffs.p = targetdata[,grep('^KSADS',colnames(targetdata))]
...
ls.models=data.frame(dependent=ls.dep.factor.combo$Var1,
                     factor=ls.dep.factor.combo$Var2,
                     covs='',stringsAsFactors = F)
ls.models$covs=paste0(c('interview_age','I(interview_age^2)','gender','race_ethnicity','site_id_l','household_income','recent_socialdprv'),
                      collapse='+') #'involuntary_unemployment','recent_socialdprv','cbcl_scr_dsm5_anxdisord_r','cbcl_scr_dsm5_adhd_r','household_income','ADHD_ever',
...
save(famRisk.Depre_bulk_measures,famRisk.Depre_items,famRisk.diffsDepre_bulk_measures,famRisk.diffsDepre_items,
     file='result/x.Supplementary_materials/FAMrisk.behav.covDemographics.RData')
