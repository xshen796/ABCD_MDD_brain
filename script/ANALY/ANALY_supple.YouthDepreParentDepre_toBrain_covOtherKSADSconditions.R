...
ls.covs=c('interview_age','recent_socialdprv','household_income','fam_highest_edu','ASR_p_depression_DSM5')#,'household_income','ADHD_ever'
...
# comorbidity
comorb.dat = readRDS('data/Behavioural/comorbid.rds')

tab.data=merge(IM.dat,tab.data,by='src_subject_id',all.x=T) %>%
      merge(.,comorb.dat,by='src_subject_id',all.x=T)
...
#cor(targetdata[,c(ls.CBCL.comorb,'CBCL.dsm5.depre.p','KSADS.Depressive_symptoms_ever.p')],method='pearson',use='complete.obs')
ls.models$covs=paste0(c('interview_age','I(interview_age^2)','sex','fsqc_qu_motion','site_id_l','race_ethnicity','recent_socialdprv','any_comorb.ksads'),collapse='+')  #,,'household_income','fam_highest_edu'
...
save(result.YouthDepre.bulk,result.YouthDepre.region.covWholeB,file='result/x.Supplementary_materials//YouthDepree_WM_subcor_FS_covKSADSotherConditions.RData')
