...
tab.data=merge(IM.dat,tab.data,by='src_subject_id',all.x=T) %>%
   mutate(meanKSADS.Depressive_symptoms_ever = rowMeans(.[,c('KSADS.Depressive_symptoms_ever.p','KSADS.Depressive_symptoms_ever.y')]))
...
# factors
ls.factor='meanKSADS.Depressive_symptoms_ever'
...
save(result.YouthDepre.bulk,result.YouthDepre.region.covWholeB,file='result/x.Supplementary_materials/meanYouthDepree_WM_subcor_FS_noSocialCov.RData')
