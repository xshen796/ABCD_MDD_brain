...
ls.models$covs=paste0(c('interview_age','I(interview_age^2)','sex','fsqc_qu_motion','race_ethnicity','recent_socialdprv','site_id_l'),collapse='+')#,,'household_income','fam_highest_edu'
...
for (i in ls.site){
    ...
    eval(parse(text=paste0('site_res.YouthDepre.bulk.',i,'=result.YouthDepre.bulk')))
    rm(result.YouthDepre.bulk)
}

ls.result=ls(pattern = '^site_res')

save(list=ls.result,file='result/x.Supplementary_materials/YouthDepree_site_BulkMeasure.RData')
